...
set.seed(202270)
 ...
 ## The following type="normal" can be replaced by type="AFT-normal" or type="AFT-normal" 
 ## if one would like to examine the Poisson or AFT model.
 
 naive = Boost_VSE(Y,Xstar,type="AFT-normal",Iter=100)$BetaHat

 correctL = SIMEXBoost(Y,Xstar,zeta=c(0,0.25,0.5,0.75,1),B=5, type="AFT-normal",sigmae=Sig,
  Iter=100, Lambda=0, Extrapolation="linear")$BetaHatCorrect
   correctL[which(abs(correctL)<0.5)]=0
 correctQ = SIMEXBoost(Y,Xstar,zeta=c(0,0.25,0.5,0.75,1),B=5, type="AFT-normal",sigmae=Sig,
