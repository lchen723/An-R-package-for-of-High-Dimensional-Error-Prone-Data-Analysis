...
set.seed(202270)
 ...
 data=ME_Data(X=X,beta=beta0,type="normal",sigmae=Sig)
 Y<-data$response
 Xstar<-data$ME_covariate

 ## The following type="normal" can be replaced by type="poisson" or type="AFT-normal" 
 ## if one would like to examine the Poisson or AFT model.
 
 naive = Boost_VSE(Y,Xstar,type="normal",Iter=100)$BetaHat

 correctL = SIMEXBoost(Y,Xstar,zeta=c(0,0.25,0.5,0.75,1),B=5, type="normal",sigmae=Sig,
  Iter=100, Lambda=0, Extrapolation="linear")$BetaHatCorrect
   correctL[which(abs(correctL)<0.5)]=0
 correctQ = SIMEXBoost(Y,Xstar,zeta=c(0,0.25,0.5,0.75,1),B=5, type="normal",sigmae=Sig,
  Iter=100, Lambda=0, Extrapolation="quadratic")$BetaHatCorrect
   correctQ[which(abs(correctQ)<0.5)]=0

#############

 which(beta0 !=0) -> Sen0; which(beta0 ==0) -> Spe0
